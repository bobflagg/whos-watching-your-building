...
app = marimo.App(width="full", title="HPD Violations — Schema Exploration")


@app.cell(hide_code=True)
def _(mo):
    mo.md("""
    # HPD Violations — Schema Exploration
    **Dataset:** HPD Violations (`wvxf-dwi5`)

    Pulls the last `LOOKBACK_MONTHS` months using `inspectiondate` as the date filter,
    saves to `data/raw/`, and documents schema, null rates, and BBL coverage.

    > **Note:** Confirm `inspectiondate` is the right filter field — alternatives include
    > `novissueddate` (notice issued) or `approveddate`. Adjust if needed.
    """)
    return
...
@app.cell
def _(fetch_recent):
    df = fetch_recent("wvxf-dwi5", "inspectiondate")
...
def _(df):
    import pathlib
    out = pathlib.Path("../data/raw/hpd_violations.parquet")
